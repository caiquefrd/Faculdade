function notaAjustadaJSON(pontuacoes) {
  ...
  for (let i = 0; i < pontuacoes.length; i++) {
    // percorre o array procurando a menorNota nota e a colocar numa variavel chamada menorNota
    if (pontuacoes[i].nota < menorNota) {
      menorNota = pontuacoes[i].nota;
    }
  }
  for (let i = 0; i < pontuacoes.length; i++) {
    //percorre novamente o array e procura a posição que bater com o valor da variavel
    if (pontuacoes[i].nota == menorNota) {
      pontuacoes[i].nota = 6;
      break;
    }
